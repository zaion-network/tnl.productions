import { Pager } from "@zaionstate/ui/";
import { createRule } from "./utils/createRule";
import { SCROLLROOT_DEFAULT } from "./utils/createCSSScrollVariable";
import { createProgressBar } from "./utils/createProgressBar";
import { createCSSScrollVariable } from "./utils/createCSSScrollVariable";
import { home } from "./pages/home";
import { bringOutScripts } from "./pages/utils/bringScriptNodeOutOfBody";
import { cleanNodeChildren } from "./pages/utils/cleanNodeChild";

const pager = new Pager({ parser: new DOMParser() });

const PROGRESSANIM = `
@keyframes progress {
  0% {
    background-color: #fff;
    width: 0%;
  }
  100% {
    background-color: rgb(20, 255, 226);
    width: 100%;
  }
}`;

const PROGRESSCLASS = `
.progress {
  height: 3px;
  width: 0%;
  background-color: #fff;
  position: fixed;
  top: 0;
  left: 0;
  animation: progress 1s linear;
}`;

// const BODY = `
// body {
//   min-height: 500vh;
// }
// `;

const body = document.getElementById("body");
bringOutScripts(body);
cleanNodeChildren(body);
const createScrollRootRule = createRule(SCROLLROOT_DEFAULT);
const createProgressAnimRule = createRule(PROGRESSANIM);
pager.route("/", () => {
  home(
    createScrollRootRule,
    createProgressAnimRule,
    createProgressBar,
    createCSSScrollVariable,
    // createPopUpMenu,
    PROGRESSCLASS
  );
});

// pager.route("/posts", async () => {
//   // while (ready === false) {}
//   posts(
//     createScrollRootRule,
//     createProgressAnimRule,
//     // createBodyRule,
//     createPosts,
//     createProgressBar,
//     PROGRESSCLASS
//   );
// });
// pager.route("/test-link-preview", () => {
//   testCreateLinkPreview(createLinkPreview);
// });

pager.resolve();

export { pager };
